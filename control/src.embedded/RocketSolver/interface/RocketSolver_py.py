...
a Python wrapper for a fast solver generated by FORCESPRO v6.0.1
   ...
   OUTPUT = RocketSolver_py.RocketSolver_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       ...
       PARAMS['all_parameters'] - matrix of size [80x1]

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['u'] - column vector of length 3

   [OUTPUT, EXITFLAG] = RocketSolver_py.RocketSolver_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = RocketSolver_py.RocketSolver_solve(PARAMS) returns 
...
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'RocketSolver/lib/RocketSolver_withModel.so'))
    csolver = getattr(_lib,'RocketSolver_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'RocketSolver/lib/libRocketSolver_withModel.so'))
    csolver = getattr(_lib,'RocketSolver_solve')

internal_mem = getattr(_lib,'RocketSolver_internal_mem')

class RocketSolver_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
RocketSolver_params = {'x0' : np.array([]),
'xinit' : np.array([]),
...
RocketSolver_params_types = {'x0' : np.float64,
'xinit' : np.float64,
'all_parameters' : np.float64,
}

class RocketSolver_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('u', ctypes.c_double * 3),
]

RocketSolver_outputs = {'u' : np.array([]),
}


class RocketSolver_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
class RocketSolver_mem(ctypes.Structure):
    pass
...
csolver.argtypes = ( ctypes.POINTER(RocketSolver_params_ctypes), ctypes.POINTER(RocketSolver_outputs_ctypes), ctypes.POINTER(RocketSolver_info), ctypes.POINTER(RocketSolver_mem), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

internal_mem.argtypes = [ctypes.c_uint]
internal_mem.restype = ctypes.POINTER(RocketSolver_mem)

def RocketSolver_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v6.0.1
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = RocketSolver_params_ctypes()
    for par in params_arg:
        ...
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=RocketSolver_params_types[par], requirements='F')
    ...
    outputs_py = RocketSolver_outputs_ctypes()
    info_py = RocketSolver_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.RocketSolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp) , _lib.RocketSolver_adtool2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.RocketSolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None , _lib.RocketSolver_adtool2forces)
            else:
                exitflag = _lib.RocketSolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp , _lib.RocketSolver_adtool2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in RocketSolver_outputs:
        RocketSolver_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return RocketSolver_outputs,int(exitflag),info_py

solve = RocketSolver_solve
