...
% 
% RocketSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the RocketSolver FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% RocketSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% RocketSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% RocketSolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function RocketSolver_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'RocketSolver_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'RocketSolver_block';
    end
    ...
    function_name = 'RocketSolver_coderFunction.m';
    position = [170, 99, 650, 231];
    parameter_sizes = struct('x0', '[160,1]',...
                             'xinit', '[13,1]',...
                             'all_parameters', '[80,1]');
    if useCompactBlock
        function_name = 'RocketSolver_coderFunctioncompact.m';
        position = [170, 99, 650, 231];
        parameter_sizes = struct('x0', '[160,1]',...
                                 'xinit', '[13,1]',...
                                 'all_parameters', '[80,1]');
