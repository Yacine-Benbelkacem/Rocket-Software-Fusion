...
% 
classdef RocketSolverBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'RocketSolver'))
                solverpath = fullfile(buildablepath, 'RocketSolver');
            end
            ForcesUpdateBuildInfo(buildInfo, cfg, 'RocketSolver', solverpath, 0, true);
        end
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_u = coder.nullcopy(zeros(3, 1));
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = RocketSolverBuildable.forcesCall(params.x0, params.xinit, params.all_parameters);
        end

        function [output,exitflag,info] = forcesCall(x0, xinit, all_parameters)
            solvername = 'RocketSolver';
            ...
            [output_c, exitflag_c, info_c] = RocketSolverBuildable.forcesInitOutputsC();
            ...
            [output, exitflag, info] = RocketSolverBuildable.forcesInitOutputsMatlab();
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_u = coder.nullcopy(double(zeros(3, 1)));
            output = struct('u', outputs_u);
